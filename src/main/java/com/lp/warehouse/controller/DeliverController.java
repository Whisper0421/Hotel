...
public class DeliverController {
    ...
    public ResultObj updateDeliver(DeliverVo deliverVo) {
        try {
            Deliver old = deliverService.getById(deliverVo.getId());
            if (!deliverVo.getStatus().equals(DeliverStatusEnum.VAL_0.getVal()) && !StringUtils.equals(old.getCarno(), deliverVo.getCarno())) {
                return new ResultObj(Constast.ERROR, "该状态下不能修改车牌号");
            }
//            if (deliverVo.getStatus().equals(DeliverStatusEnum.VAL_1.getVal()) && !old.getStatus().equals(DeliverStatusEnum.VAL_0.getVal())) {
//                return new ResultObj(Constast.ERROR, "该状态不能修改为待发货");
//            }
            this.deliverService.update(deliverVo);
